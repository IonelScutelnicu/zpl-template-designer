...
- **Embeddable** — integrate the editor into other web apps as an iframe or new tab with a tiny SDK, two-way postMessage data flow, host-controlled panel visibility, and host-supplied preview fonts matched to the fonts a template names. The host can swap the open document at any time (`setZpl` / `loadZPL` / `loadTemplate`) with no reload and no re-sending of fonts — the swap lands as one undoable step — and open a named settings panel with `focusPanel`. See [docs/EMBEDDING.md](docs/EMBEDDING.md)
...
- **Resident Zebra fonts** — scalable Font 0 and all 15 resident bitmap fonts A–H and P–V, with each bitmap font constrained to Zebra's [documented height/width magnification grid](https://docs.zebra.com/us/en/printers/software/zpl-pg/fonts-and-barcodes/font-matrices.html). P–V are the same CG Triumvirate Bold Condensed face as Font 0 at fixed cell sizes, so they preview through Font 0's calibrated substitute rather than a bundled font file. Zebra's 6 dot/mm matrix does not list P–V, so verify support on the target printer and firmware at that density
